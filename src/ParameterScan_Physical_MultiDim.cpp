...
int main(int argc, char* argv[]) {
  ...
  if ( argc != 13)
  {
	 printf("ParameterScan_Physical_MultiDim usage:\n");
	 printf("ParameterScan_Physical_MultiDim <tag> <BitWriteLHA> <mh> <mH> <mA> <mHc> <sba> <tanb> <l6> <l7> <m12_2> <yukawa-type>");
  }
  ...
  double mA_in     = (double)atof(argv[5]);
  double mHc_in    = (double)atof(argv[6]);
  double sba_in    = (double)atof(argv[7]);
  double tanb_in   = (double)atof(argv[8]);
  double l6_in     = (double)atof(argv[9]);
  double l7_in     = (double)atof(argv[10]);
  ...
  bool pset = model.set_param_phys(mh_in,mH_in,mA_in,mHc_in,sba_in,l6_in,l7_in,m12_2_in,tanb_in);
...
// Run HiggsBounds 'full', i.e. with each Higgs result separately  
  ...
  // Debuggg 
  // double mh,mH,mA,mHc,sba,lambda6,lambda7,tan_beta,m12_2;
  // model.get_param_phys(mh,mH,mA,mHc,sba,lambda6,lambda7,m12_2,tan_beta);
  ...
  std::string line = Form("%.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6f %.6e %d %d %d %.6f %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e %.6e\n", mh_in, mH_in, cba, tanb, Z4, Z5, Z7, csqtot, tot_hbobs, BitAllowedStability, BitAllowedUnitarity, BitAllowedPerturbaticity, mA_in, Gamma_h, Gamma_A, mHc_in, l6_in, l7_in, m12_2_in, S, T, U, V, W, X );
